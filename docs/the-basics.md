...
To connect an internet domain to an application running locally on port 3128 open a command terminal and run:

```bash
ssh -R 80:localhost:3128 localhost.run
```
  ...
  return (
    <>
      ...
      <pre><code parentName="pre" {...{
              "className": "bash"
            }}>{`ssh -R 3128:localhost:${port} localhost.run
`}</code></pre>
    </>
  )
};
